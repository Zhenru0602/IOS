...
final class CoreManager {
    ...
    func save(with city: City) {
        if !isInCoreData(city: city){
            //remove if limit was reached
            removeIfReachLimit()
            ...
            print("\(city.name), \(city.state) Already In Core Data")
        }
    }
    ...
    func isInCoreData(city: City) -> Bool{
        let coreCities = getCoreCities()
        for i in coreCities{
            if city.name == i.name && city.state == i.state{
                return true
            }
        }
        return false
    ...
    func removeIfReachLimit(){
        let coreCities = getCoreCities()
        if coreCities.count >= 10{
            context.delete(coreCities[0])
            print("Limit Is Reached, \(coreCities[0].name ?? ""), \(coreCities[0].state ?? "") Was Deleted")
            saveContext()
        }   else{
